        ...
        <div class="bg-white shadow rounded-lg d-block d-sm-flex">
            <div class="tab-content p-4 p-md-5" id="v-pills-tabContent">
                <div class="tab-pane fade show active" id="compte" role="tabpanel" aria-labelledby="compte-tab">
                    ...
                    <form action="?controller=planning&action=remove&id_creneau=<?=$id_creneau?>&nom=<?=$nom?>&deb=<?=$heure_d?>&fin=<?=$heure_f?>&jour=Vendredi" method="post">
                        <h4>Exposant : <?= $nom ?></h4>
                        <p><strong>Créneaux : </strong><?=$heure_d?> - <?=$heure_f?></p>
                        <p><strong>Jour : </strong>Vendredi</p>
